...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout,
                            ...
                            QSizePolicy, QHeaderView, QSplitter, QComboBox,
                            QCheckBox)
...
class XRDDataAnalyzer(QMainWindow):
    ...
    GROUP_STYLE_TPL = "QGroupBox {{ border: 1px solid {}; font-weight: bold; padding-top: 15px; font-family: Arial; font-size: 10pt; margin-top: 2ex; }}"
    
    LABEL_STYLE = "font-family: Arial; font-size: 10pt;"
    HEADER_STYLE = "font-family: Arial; font-size: 10pt; font-weight: bold;"
    INPUT_STYLE = """
        QLineEdit, QComboBox, QTableWidget, QHeaderView::section, QLabel {{
    ...
    def __init__(self):
        ...
        # Set scalable font for the application
        font = QFont("Arial", 10)
        self.setFont(font)
    ...
    def init_ui(self):
        ...
        contributors_label.setStyleSheet("color: #666666; font-weight: bold; font-family: Arial; font-size: 10pt; margin-bottom: 10px;")
        contributors_label.setAlignment(Qt.AlignRight | Qt.AlignTop)
        contributors_label.setSizePolicy(QSizePolicy.Preferred, QSizePolicy.Fixed)
        main_layout.addWidget(contributors_label, stretch=0)
        
        # Top control area (30% height)
        top_group = QGroupBox("Data Control")
        top_group.setStyleSheet("QGroupBox { font-weight: bold; padding-top: 20px; font-family: Arial; font-size: 10pt; }")
        ...
        # 使用初始化后的sizeHint设置最大高度
        from PyQt5.QtCore import QTimer
        QTimer.singleShot(0, lambda: data_group.setMaximumHeight(data_group.sizeHint().height()))
        ...
        self.import_btn.setStyleSheet(self.BUTTON_STYLE_TPL.format(self.UI_COLORS['import'][0], self.UI_COLORS['import'][1]))
        self.import_btn.setToolTip("Click to import XRD data file")
        self.skip_last_row_checkbox = QCheckBox("Skip last row")
        self.skip_last_row_checkbox.setChecked(self.settings.value("import/skip_last_row", True, type=bool))
        self.skip_last_row_checkbox.setToolTip("Use this for SSRF files whose last row is a non-data trailer")
        ...
        data_layout.addWidget(self.skip_last_row_checkbox)
        ...
        # 设置固定垂直大小策略，保持高度不变
        adj_group.setSizePolicy(QSizePolicy.Preferred, QSizePolicy.Fixed)
        # 使用初始化后的sizeHint设置最大高度
        QTimer.singleShot(0, lambda: adj_group.setMaximumHeight(adj_group.sizeHint().height()))
        ...
        self.adjust_btn.setStyleSheet(self.BUTTON_STYLE_TPL.format(self.UI_COLORS['calibration'][0], self.UI_COLORS['calibration'][1]))
        self.adjust_btn.setToolTip("Apply energy calibration parameters")
        
        self.export_btn = QPushButton("Export Data")
        self.export_btn.setStyleSheet(self.BUTTON_STYLE_TPL.format(self.UI_COLORS['export'][0], self.UI_COLORS['export'][1]))
        ...
        # 设置固定高度策略，确保整个顶部区域高度不受窗口大小变化影响
        top_group.setSizePolicy(QSizePolicy.Preferred, QSizePolicy.Fixed)
        
        # 设置一个固定高度，避免拉伸
        top_group.setMaximumHeight(top_group.sizeHint().height())
        ...
        """ % (self.UI_COLORS['roi'][0], self.UI_COLORS['roi'][0], self.UI_COLORS['roi'][1]))
        self.select_btn.setToolTip("Select region of interest (ROI)")
        
        self.fit_btn = QPushButton("Fit Peaks")
        self.fit_btn.setStyleSheet(self.BUTTON_STYLE_TPL.format(self.UI_COLORS['fit'][0], self.UI_COLORS['fit'][1]))
        self.fit_btn.setToolTip("Perform Gaussian fit on selected ROI")
        
        self.clear_btn = QPushButton("Clear All")
        self.clear_btn.setStyleSheet(self.BUTTON_STYLE_TPL.format(self.UI_COLORS['clear'][0], self.UI_COLORS['clear'][1]))
        ...
        # Set table font and resize policy
        font = QFont("Arial", 10)
        self.calib_table.setFont(font)
        header_font = QFont("Arial", 10)
        header_font.setBold(True)
        self.calib_table.horizontalHeader().setFont(header_font)
        self.calib_table.verticalHeader().setFont(font)
        # 确保表格中输入的内容也使用Arial字体
        self.calib_table.setStyleSheet("QTableWidget { font-family: Arial; font-size: 10pt; }")
        ...
        if self.settings.contains("calibTable/columnWidths"):
            ...
            for col, width in enumerate(widths):
                self.calib_table.setColumnWidth(col, int(width))
        else:
            # Set default widths
        ...
        self.calib_table.setStyleSheet(self.TABLE_STYLE.format(self.UI_COLORS['theta'][0]))
        
        self.calibrate_btn = QPushButton("Calibrate 2θ")
        self.calibrate_btn.setStyleSheet(self.BUTTON_STYLE_TPL.format(self.UI_COLORS['theta'][0], self.UI_COLORS['theta'][1]))
        
        self.export_calib_btn = QPushButton("Export Calibration")
        self.export_calib_btn.setStyleSheet(self.BUTTON_STYLE_TPL.format(self.UI_COLORS['export_calib'][0], self.UI_COLORS['export_calib'][1]))
        ...
        self.result_table.verticalHeader().setFont(font)
        # 确保结果表格中的内容也使用Arial字体
        self.result_table.setStyleSheet("QTableWidget { font-family: Arial; font-size: 10pt; }")
        ...
        splitter.setSizes([int(self.width()*0.3), int(self.width()*0.7)])  # Left 30%, Right 70%
        ...
        self.statusBar().setStyleSheet(self.LABEL_STYLE)
    ...
    def plot_data(self):
        ...
        for i, roi in enumerate(dataset.selected_regions):
            ...
            if not is_fitted:
                ...
                self.ax.text(roi['x_min'] + roi['width']/2, 
                            ...
                            color=roi['color'], ha='center', va='top', fontsize=10)
        ...
        # Set axis labels, tick labels and legend
        self.ax.set_xlabel('Energy (keV)' if dataset.x_axis_adjusted else 'Channel', fontname='Arial', fontsize=10)
        self.ax.set_ylabel('Intensity', fontname='Arial', fontsize=10)
        self.ax.tick_params(axis='both', which='major', labelsize=10)
        
        if legend_handles:
            self.ax.legend(legend_handles, legend_labels, loc='upper right', 
                         framealpha=0.7, fancybox=True, prop={'family': 'Arial', 'size': 10})
    ...
    def on_motion(self, event):
        ...
        if event.inaxes == self.ax:
            ...
            self.current_roi['width'] = max(0.001, x_max - x_min)
            y_min, y_max = self.ax.get_ylim()
            height = y_max - y_min
    ...
    def fit_peaks(self):
        ...
        for roi in dataset.selected_regions:
            ...
            roi_x = x_data[mask]
            roi_y = y_data[mask]
            if len(roi_x) < MIN_FIT_POINTS:
                QMessageBox.warning(
                    self,
                    "Warning",
                    f"ROI {roi['x_min']:.3f}-{roi['x_max']:.3f} has fewer than {MIN_FIT_POINTS} points"
                )
                continue
            
            # Initial guess for parameters
            peak_idx = np.argmax(roi_y)
            a_guess = max(roi_y[peak_idx] - np.min(roi_y), np.finfo(float).eps)  # Peak height above baseline
            x0_guess = roi_x[peak_idx]  # Peak center
            roi_width = max(roi['x_max'] - roi['x_min'], np.finfo(float).eps)
            sigma_guess = roi_width / 6  # Width estimate
            ...
            bounds = ([0, roi['x_min'], np.finfo(float).eps, -np.inf, -np.inf], 
                     [np.inf, roi['x_max'], roi_width, np.inf, np.inf])
    ...
    def calibrate_2theta(self):
        ...
        try:
            ...
            for d, E in pairs:
                ...
                arg = HC_KEV_ANGSTROM / (2 * d * E)
                if not 0 < arg <= 1:
                    raise ValueError(f"Invalid Bragg input d={d:.5f}, E={E:.5f}: asin argument={arg:.6f}")
                twotheta_deg = 2 * np.degrees(np.arcsin(arg))
                twotheta_values.append(twotheta_deg)
            
            # Calculate average and standard deviation
            avg_2theta = np.mean(twotheta_values)
            std_2theta = np.std(twotheta_values, ddof=1) if len(twotheta_values) > 1 else 0.0
            residuals = [value - avg_2theta for value in twotheta_values]
            
            # Store calibration parameters
            self.calibrated = True
            self.calib_params = {
                'avg_2theta': avg_2theta,
                'std_2theta': std_2theta,
                'hc': HC_KEV_ANGSTROM,
                'values': twotheta_values,
                'residuals': residuals,
            }
            ...
            if len(pairs) > 1:
                max_abs_resid = max(abs(value) for value in residuals)
                status_text += f"\nmax residual = {max_abs_resid:.5f}°"
        ...
        if filename:
            try:
                ...
                with open(filename, 'w') as f:
                    ...
                    for idx, (d, E, remarks) in enumerate(d_e_pairs):
                        twotheta = values[idx] if idx < len(values) else np.nan
                        residual = residuals[idx] if idx < len(residuals) else np.nan
                        f.write(f"{d:.5f}\t{E:.5f}\t{twotheta:.5f}\t{residual:.5f}\t{remarks}\n")
    ...
    def closeEvent(self, event):
        ...
        try:
            ...
            self.settings.setValue("import/skip_last_row", self.skip_last_row_checkbox.isChecked())
        except:
            pass
    ...
    def import_data(self):
        ...
        if filename:
            try:
                ...
                # SSRF exports may include a non-data trailer row; keep this user-controllable.
                if self.skip_last_row_checkbox.isChecked() and len(raw_data) > 0:
                    raw_data = raw_data[:-1]
